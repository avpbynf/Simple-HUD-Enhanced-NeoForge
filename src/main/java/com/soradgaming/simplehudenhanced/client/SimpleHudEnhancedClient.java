...
public class SimpleHudEnhancedClient {
    ...
    // 1.21.11 introduces the KeyMapping.Category object (Yarn KeyBinding.Category); the older
    // String translation-key category no longer exists. Register this category alongside the
    // key mapping so the controls screen groups the binding under a dedicated heading.
    ...
    private void registerGuiLayers(RegisterGuiLayersEvent event) {
        ...
        // build's HUD element ordering. Modded layers registered through this event are inserted
        // raw: unlike vanilla layers they are NOT wrapped in the hideGui guard, so F1 (hide GUI)
        // and F3 (debug overlay) are checked explicitly here. Unlike upstream, the config value is
        // never mutated or saved when F3 opens/closes: the HUD is masked at render time only.
